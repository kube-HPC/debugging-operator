...
class KubernetesApi extends EventEmitter {
    ...
    async createAlgorithmForDebug({ deploymentSpec, ingressSpec, serviceSpec }) {
        log.debug(`Creating serivces ${deploymentSpec.metadata.name}`, { component });
        try {
            try {
                
                const resDepolyment = await this._client.apis.apps.v1.namespaces(this._namespace).deployments.post({ body: deploymentSpec });
            } catch (error) {
                console.log(`unable to create deployment ${deploymentSpec.metadata.name}. error: ${error.message}`, { component }, error);
            }
            try {
                const resIngress = await this._client.apis.extensions.v1beta.namespaces(this._namespace).ingresses.post({ body: ingressSpec });
            ...
            try {
                const resService = await this._client.apis.v1.namespaces(this._namespace).services.post({ body: serviceSpec });
    ...
    async updateAlgorithmForDebug({ deployment, ingress, service }) {
        log.debug(`Upating kubernetes ${spec.metadata.name}`, { component });
        try {
            const resDepolyment = await this._client.apis.apps.v1.namespaces(this._namespace).ingresses.patch({ body: deployment });
         //   const resIngress = await this._client.apis.apps.v1.namespaces(this._namespace).deployments.patch({ body: ingress });
        //    const resService = await this._client.apis.v1.namespaces(this._namespace).services.patch({ body: service });
            return {
                resDepolyment,
      //  //        resIngress,
                resService
