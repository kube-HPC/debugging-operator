...
const _updateDeployment = async (algorithmName, options) => {
    const { deploymentSpec} = createKindsSpec({ algorithmName, ...options });
    log.debug(`need to add ${algorithmName} with details ${JSON.stringify(options, null, 2)}`, { component });
    const spec = createKindsSpec({ algorithmName, ...options });
    const deploymentCreateResult = await kubernetes.updateAlgorithmForDebug({ deploymentSpec });
    return deploymentCreateResult;
};
